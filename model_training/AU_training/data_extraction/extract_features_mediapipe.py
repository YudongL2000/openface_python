...
def load_all_images_DISFA(action_unit_label_path, facial_raw_images_path):
    Left_images = {}
    Right_images = {}
    ...
    faces_left = load_image_paths(left_images_folder, facial_raw_images_path)
    faces_right = load_image_paths(right_images_folder, facial_raw_images_path)
...
def landmark_detection(image_path, image_landmark_path, write_images=False, display= False):
    ...
    if (write_images):
        cv2.imwrite(image_landmark_path, image)
    if display:
        cv2.imshow(image)
    return landmarks
...
def store_landmarks(image_key, store_landmark_image_path, store_landmark_jsons, Left_images, Right_images):
    ...
    left_landmarks = landmark_detection(left_image_path, left_save_path)
    right_landmarks = landmark_detection(right_image_path, right_save_path)
...
def image_feature_extraction_DISFA(facial_raw_images_path, action_unit_label_path, store_landmark_jsons_path):
    store_landmark_image_path = "tmp/DISFA/landmarks"
    Left_images, Right_images = load_all_images_DISFA(action_unit_label_path, facial_raw_images_path)
    for image_key in Left_images.keys():
        store_landmarks(image_key, store_landmark_image_path=store_landmark_image_path, store_landmark_jsons=store_landmark_jsons_path, Left_images=Left_images, Right_images=Right_images)
